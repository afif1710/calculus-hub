...
const Index = () => {
  ...
  const { favorites, recent, toggleFavorite, isFavorite, addToRecent } =
    useFavoritesRecent();
  ...
  const handleSelectCalculator = useCallback(
    (categoryId: string, calcId: string) => {
    ...
    <div className="min-h-screen bg-background">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-muted-foreground hidden sm:block">
                  80+ calculators at your fingertips
                </p>
              </div>
            </div>
            <ThemeSwitcher />
      ...
      <section className="container py-12">
        ...
        >
          <h2 className="text-4xl sm:text-5xl font-bold mb-4">
            Every calculator & Converter you need,{" "}
            <span className="gradient-text">in one place</span>
          </h2>
          <p className="text-lg text-muted-foreground">
            Finance, math, health, developer tools & more.
          </p>
        </motion.div>
        <motion.div
              ...
              (calc) =>
                calc && (
                  <button
                    key={calc.id}
                    onClick={() =>
                      handleSelectCalculator(calc.categoryId, calc.id)
                    }
            ...
            >
              <CategoryCard
                ...
                onSelectCalculator={(calcId) =>
                  handleSelectCalculator(category.id, calcId)
                }
                selectedCalcId={
                  selectedCalc?.categoryId === category.id
                    ? selectedCalc.id
                    : null
                }
      ...
      <footer className="border-t border-border py-6">
        <div className="container text-center text-sm text-muted-foreground">
          <p>
            Press{" "}
            <kbd className="px-1.5 py-0.5 rounded bg-secondary text-xs">⌘K</kbd>{" "}
            to search •{" "}
            <kbd className="px-1.5 py-0.5 rounded bg-secondary text-xs">T</kbd>{" "}
            to change theme
          </p>
        </div>
      </footer>
